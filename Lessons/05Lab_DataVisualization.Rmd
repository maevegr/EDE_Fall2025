...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
   ...
   #option 2. Doesn't actually work but it's something like this
...
#Add a title
surface_temp_1999_plot + 
  ggtitle("Surface temperature, 1999") + #Title for the graph
  labs(color='Lake') +  #Legend title
  theme_wsj()
```
...
PeterPaul.chem.nutrients %>% 
  ...
  geom_point()+
    facet_wrap(facet = vars(lakename) ,nrow = 1)+ #vars  = what are the variables you want to separate by
  ylab("Temperature")+
  labs(color="year")
...
PeterPaul.chem.nutrients %>% 
  ...
  geom_point(size = .5)+
  scale_x_date(date_breaks = "5 years", date_labels = "%Y")+ #can add limits here but don't need to
...
EPAair %>% 
  ...
  geom_smooth(method = "lm", se=FALSE) #linear model/ if se = TRUE the line would have a range
