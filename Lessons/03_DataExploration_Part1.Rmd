...
> Answer: shows what it's installing, the URL it's trying, content type and 
#confirms the version of the package that it installs
...
USGS.flow.data <- read.csv(here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"),
                        stringsAsFactors = TRUE)
...
> ANSWER: this file is found in the data --> metadata folder. It could serve as a guidebook for future users to understand why you manipulated the data in certain ways (which columns are which, how the data was collected, why there may be gaps in the data, etc)
...
> Answer: They are number type data listed as "NA"
...
> Answer: There may be data that considers NA to be 0. Or for qualitative data, if a blank 
may be a respondent choosing to not respond to a question, which may be helpful 
to consider
