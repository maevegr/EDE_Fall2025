...
# Fix this code
#Need to put color outside of aethetics
...
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) +
  geom_line() #geom_line instead of geom_point
print(PMplot.line)
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #values will be plotted in descending order so that deeper values are on the bottom and more shallow values are on the top
print(tempvsdepth)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Fill would change the color of the box (but we want the points colored)
print(Nutrientplot3)
...
# Using a gathered dataset Shows the same thing but uses the gathered version so theres one column with nutrient type and one with value. Adds a legend automatically and easier to code
Nutrientplot7 <-   
...
# Ridgeline plot Helps us see variation between the sites
Nutrientplot6 <-
