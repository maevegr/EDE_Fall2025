...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #separator is space " ", 3 columns because space-month-space-average, call them null, month, null2
  mutate( Date = my(paste0(Month,"-",Year))) %>% #combine the month and year, my() converts the character into a date
  select(Date,Rainfall) #select the two columns I need
...
#don't need to do the zoo stuff because we got BR_complete summary without NAs
Rainfall_BR_processed_2 <- 
  Rainfall_BR_processed %>% 
  mutate( Rainfall.clean = zoo::na.approx(Rainfall))

ggplot(Temp_BR_processed, aes(x = Date, y = Temperature_C)) +
  geom_line()

#create a data frame with all the months in the time frame
Months <- as.data.frame(seq.Date(from=as.Date("1901/01/01"),to = as.Date("2016/12/01"), by="month"))

#rename the column Date
colnames(Months)<-"Date"

#join with an earlier table that has the month with rainfall and temp for each month
BR_complete <- left_join(Months, BR_complete)

#if there are missing temp and or rainfall values we would get an NA for mean
summary(BR_complete)
...
rainfall_data_ts <- ts(Rainfall_BR_processed$Rainfall,
                   start=c(f_year,f_month),
                   frequency=12) 


Rainfall_decomp <- stl(rainfall_data_ts,s.window = "periodic")
plot(Rainfall_decomp)

#temperature
f_month1 <- month(first(Temp_BR_processed$Date))
f_year1 <- year(first(Temp_BR_processed$Date))
temp_data_ts <- ts(Temp_BR_processed$Temperature_C,
                   start=c(f_year1,f_month1), #could also be c(1901,1)
                   frequency=12) 


temp_decomp <- stl(temp_data_ts,s.window = "periodic")
plot(temp_decomp)
...
#create a data frame with one column for each trend
temp_Components <- as.data.frame(temp_decomp$time.series[,1:3])

#add the date and actual value to the data frame
temp_Components_date <- mutate(temp_Components,
