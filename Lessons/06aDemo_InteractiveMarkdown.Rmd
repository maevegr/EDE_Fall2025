...
#Construct the plot
my_plot1 <- nutrient_data %>% 
  filter(year4 > start_year & year4 < end_year) %>% 
  ggplot(aes(
    x = sampledate,
    y = !!sym(the_parameter), #need to call the parameter within the dataset that is piped in above
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) +
  scale_x_date(
...
#Construct the plot
  nutrient_data %>% #don't save as a variable, just plot it
    filter(year4 > start_year & year4 < end_year) %>% 
    ggplot(aes(
      x = sampledate,
      y = !!sym(the_parameter), #need to call the parameter within the dataset that is piped in above
