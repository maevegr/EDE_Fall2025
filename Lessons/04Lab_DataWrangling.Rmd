...
NTL.phys.data.90_99 <- NTL.phys.data %>% 
  filter(year4 %in% c(1990:1999), #don't need the vector notation c()
...
> Answer: 
This isn't a part of our data. Can change if for example we sorted by lake name
---
...
ex3 <- NTL.phys.data %>% 
  ...
  filter(lakename =="Tuesday Lake"& 
           ...
           month ==7) #could also just add month(sampledate)==7 and not use mutate
...
ex4 <- ex3 %>% 
  filter(depth==0) %>% 
  summarise(mean_surface_temp = mean(temperature_C)) #summarise returns a new data frame with the values for each function input
...
ex5<- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irradiance_Value")
