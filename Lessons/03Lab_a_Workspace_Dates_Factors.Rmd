...
ChemistryPhysics = read.csv(
      file = here('Data','Raw', 'NTL-LTER_Lake_ChemistryPhysics_Raw.csv'), stringsAsFactors = TRUE)

# 2.
library(tidyverse);library(here)

RawFolder <- here("Data", "Raw")
EPAair_03_Files <- dir(RawFolder, pattern = "EPAair_O3*.*", full.names = TRUE) #* means anything after . is included in list; full.names = TRUE looks and prints the full path. This is important for the next step
EPAair_03_Data <- EPAair_03_Files %>% map(read.csv) %>% bind_rows() #combines the files in the list created above to one data frame
```
...
#Since the inpu format is year-month-day we will use function ymd() which will give us the R default of yyyy-mm-dd
date_obj_today <- ymd(str_today)
date_obj_today

#Ex2
str_today2 <- "Feb 7, 2023"
#Since the input format is month-day-year we will use function mdy()
...
#Read in data, don't convert strings to factors. Converting strings to factors  allows R to group names that are the same.
...
#Summary - is it meaningful? Yes because giving a count (not mean etc which isn't helpful for the day of a week)
summary(sites$dow)

#Show the levels associated with our factor column. Basically groups of factors
levels(sites$dow)
```
