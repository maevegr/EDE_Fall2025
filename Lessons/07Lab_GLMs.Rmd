...
EPAair %>% 
  ...
  stat_summary(geom = "text",
               fun = max, #fun = function. which statistic do you want to display
               vjust = -1, #vertical justification (places above point)
               size = 4, #text size
               label = c ("b", "a")
               )+
  ylim(0,150)
...
#only 13% of the variability of ozone is explained by PM2.5
...
EPAair %>% 
  ggplot(
    aes(x = PM2.5,
        y = Ozone)) +
  geom_point()+
  geom_smooth(method = lm)+ #add se = FALSE to get rid of variance buffer on line
  geom_text(
    x = 25, #x coordinate location of the text
    y = 130, # y coordinate location of the text
    label = expression("PM2.5 = 0.38*Ozone +15.64")  #values from the summary of the regression
  )
...
Air.subset <- 
  ...
  na.omit()
Air.corrplot <- cor(Air.subset)
corrplot(Air.corrplot, method = "ellipse")
 #mean_Lat might help explain PM2.5 (pos correlation). Month might help explain Ozone (neg correlation)
...
#OzonebyAll.regression <- lm(data = , PM2.5 ~ + Ozone + Year + Month + mean_Lat + mean+Lng)
#compares correlation of all variables with PM2.5
...
#adding more variables to the regression increases the R2 value to 19%
