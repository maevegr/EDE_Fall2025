...
kable(summary_PM25, 
      col.names = c("County","$\\mu g/m^3$"), #column names
      caption = "Mean Particulates (2.5mm)", #Title
      digits = 2) #round to 2 digits

kable(summary_Ozone, 
      col.names = c("County","$\\mu g/m^3$"), #column names
      caption = "Mean Ozone", #Title
      digits = 2) #round to 2 digits
...
```{r echo=FALSE, fig.cap="2018 Ozone Levels by Month", fig.align="left", fig.height=3}
Ozone_2018 <- epa_data %>%
  drop_na(Ozone) %>%
  filter(Year == 2018)%>%
  ggplot( 
    aes(
      ...
      #change month data from int to factor and label by corresponding month 
      #name
      x=factor(Month,levels = 1:12,labels = month.abb), #x value
      y = Ozone #y value
    )
  )+
  xlab("Month")+ #x axis label
  ylab("Concentration")+ #y axis label
  ggtitle("2018 Ozone Concentration by Month")+ #plot title
  geom_boxplot() #boxplot
Ozone_2018

  
```
```{r echo=FALSE, fig.cap="2019 Ozone Concentration by Month", fig.align="left", fig.height=3}
Ozone_2019 <- epa_data %>%
  drop_na(Ozone) %>%
  filter(Year == 2019)%>%
  ggplot( 
  ...
  xlab("Month")+
  ylab("Concentration")+
  ggtitle("2019 Ozone Concentration by Month")+
  geom_boxplot()
Ozone_2019
```
...
Maeve Gualtieri-Reed 10/19/2025

## Git Repository
https://github.com/maevegr/EDE_Fall2025
